...
export function buildRecipientShareMessage(giftUrl: string): string {
  return `🎉 קיבלתי מתנה מדהימה!\n\nגם אתם יכולים לנסות:\n${giftUrl}`;
}

export function buildSenderShareMessage(giftUrl: string, title?: string): string {
  const heading = title ? `🎁 ${title}` : "🎁 יש לך הפתעה!";
  return `${heading}\n\nגרד את הכרטיס וגלה מה מחכה לך:\n${giftUrl}`;
}
